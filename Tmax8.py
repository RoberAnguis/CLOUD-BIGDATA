...
df3 = df2.groupBy("Año").max('avg(Día 1)', 'avg(Día 2)', 'avg(Día 3)', 'avg(Día 4)', 'avg(Día 5)', 'avg(Día 6)', 'avg(Día 7)', \
...
t_max_8_max = df3.rdd.map(max_month) 
df_res = spark.createDataFrame(t_max_8_max).toDF("Año","Temp_Max") # reconvertimos a df
df_res.write.mode("overwrite").option("header", "true").option("sep",";").csv("Tmax8_results.csv") # guardamos en csv

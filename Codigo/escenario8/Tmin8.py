...
df = spark.read.option("header", "true").option("sep",";").schema(schema).csv("Tmin8.csv")
...
def min_month(line): 
	min = 50  # mas alta que cualquier temperatura de csv
	for i in range (1, 32):
...
t_max_8_max = df3.rdd.map(min_month) 
df_res = spark.createDataFrame(t_max_8_max).toDF("Año","Temp_Min") # reconvertimos a df
df_res.write.mode("overwrite").option("header", "true").option("sep",";").csv("Tmin8_results.csv") # guardamos en csv

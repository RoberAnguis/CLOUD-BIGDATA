...
t_max_avg_8 = dfMax2.rdd.map(average).reduceByKey(lambda a,b: ("basura", (a[1]+b[1]), (a[2]+b[2]))).map(lambda x: (x[0], x[1][1]/x[1][2]))
t_max_avg_8.collect()

t_min_avg_8 = dfMin2.rdd.map(average).reduceByKey(lambda a,b: ("basura", (a[1]+b[1]), (a[2]+b[2]))).map(lambda x: (x[0], x[1][1]/x[1][2]))
t_min_avg_8.collect()

# hacer la media entre ambos
rdd_mix1 = t_max_avg_8.join(t_min_avg_8)
...
df_res.write.mode("overwrite").option("header", "true").option("sep",";").csv("Desv8_results.csv") # guardamos en csv
